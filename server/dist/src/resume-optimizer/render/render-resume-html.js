...
function renderResumeHtml(resume) {
    ...
    if (resume.basics.email)
        contactParts.push(esc(resume.basics.email));
    if (resume.basics.phone)
        contactParts.push(esc(resume.basics.phone));
  ...
  <span class="exp-date">${esc(exp.startDate)} – ${exp.current ? 'Present' : esc(exp.endDate)}</span>
    ...
    if (resume.education.length) {
        ...
        for (const edu of resume.education) {
            ...
            if (edu.score)
                html += `  <div class="score">${esc(edu.score)}</div>\n`;
            html += `</div>\n`;
        }
    }
...
//# sourceMappingURL=render-resume-html.js.map