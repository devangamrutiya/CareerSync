...
export async function validateSuggestions(input: {
...
// ─── Code-based validation (no AI) ───────────────────────

export function validateNoDateMutation(
  original: ResumeJson,
  suggestions: ResumeSuggestionsJson,
): string[] {
  ...
  for (const expSuggestion of suggestions.experienceSuggestions) {
    const origExp = original.experience.find(
      (e) => e.id === expSuggestion.experienceId,
    );
    if (!origExp) {
      warnings.push(
        `Experience suggestion references unknown id: ${expSuggestion.experienceId}`,
      );
    }
  }
  ...
  for (const projSuggestion of suggestions.projectSuggestions) {
    const origProj = original.projects.find(
      (p) => p.id === projSuggestion.projectId,
    );
    if (!origProj) {
      warnings.push(
        `Project suggestion references unknown id: ${projSuggestion.projectId}`,
      );
    }
  }
